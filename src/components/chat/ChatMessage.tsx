...
export function ChatMessage({
  ...
  // Streaming animation for agent messages
  ...
  useEffect(() => {
    ...
    // Only animate agent messages once
    ...
    let currentIndex = 0
    const charsPerFrame = Math.max(2, Math.floor(fullText.length / 50)) // Adaptive speed
      ...
      >
        <div className="relative rounded-lg bg-amber-50/50 px-4 py-3 dark:bg-amber-900/10">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: markdownToHtml(displayedText) }}
          />
          {isStreaming && (
            <motion.span
              className="ml-0.5 inline-block h-3 w-0.5 bg-amber-600 dark:bg-amber-400"
    ...
    >
      ...
      <div className="flex-1 space-y-2 overflow-hidden">
        ...
        <div className="relative">
          ...
          {isStreaming && (
            <motion.span
              className="ml-0.5 inline-block h-4 w-0.5 bg-neutral-900 dark:bg-neutral-100"
              animate={{ opacity: [1, 0, 1] }}
              transition={{ duration: 0.8, repeat: Infinity }}
            />
          )}
